"""Qwen3-Reranker-4B client for OpsCompile.

Updated 2026-04-28: align with the **vLLM-specific Qwen3-Reranker
example** that hits ``/v1/rerank`` directly. The server applies the
yes/no head internally and returns ``relevance_score`` per document —
no client-side chat template or logprob parsing needed.

Architecture
------------
For each (query, doc) pair the model judges "yes" / "no" given a
system instruction. The vLLM ``/v1/rerank`` endpoint expects the
caller to provide:

  * ``query``     — already wrapped with the chat-template prefix +
                    ``<Instruct>: …<Query>: …`` lines, ending with a
                    trailing newline.
  * ``documents`` — each document already wrapped with
                    ``<Document>: …`` plus the chat-template suffix
                    that primes the assistant for yes/no.

The server then concatenates ``query + doc`` per pair, runs forward
once with ``max_tokens=1``, and returns the softmax-normalised
``relevance_score`` (yes-probability) per document.

This is strictly an HTTP wrapper. ``format_instruction`` and
``process_inputs`` are kept for offline parity with the official
HuggingFace README — those build vLLM ``TokensPrompt`` objects for
in-process ``LLM.generate(...)`` and use the same chat template.

References
----------
- https://huggingface.co/Qwen/Qwen3-Reranker-4B  (README;
  ``format_instruction`` / ``process_inputs`` / ``compute_logits``)
- vLLM Qwen3-Reranker /v1/rerank example (the prefix / suffix /
  template literals below are taken verbatim from that example).
"""
