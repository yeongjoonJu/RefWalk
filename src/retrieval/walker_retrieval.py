"""Walker retrieval pipeline — topic anchoring + seed retriever + OKG expand + multi-view reranker.

Default flow (Round-5, post 2026-05 multi-view fusion analysis):

  1. **Query anchoring** — accept either a raw question string or a
     pre-anchored dict. Raw strings are fed to ``topic_extractor`` to
     produce ``{topic, conditions: {actor, temporal, magnitude,
     situational}}`` (mirrors ``cli/test_topic_anchoring.py``). The
     extractor performs a live LLM call every time — there is no
     cache layer; callers that want reproducibility (e.g. the eval
     CLI) pass an already-anchored dict instead.
  2. **Seed retrieval** — dense top-N (``rerank_pool``, default 50)
     over the corpus using the tagged seed query
     ``[TOPIC] … [Q] … [ACTOR] … [TEMPORAL] …`` (mid view).
  3. **OKG typed-edge expansion** — 1-hop along ``expand_edge_types``
     (REFERENCES / DELEGATES_TO / SPECIFIES) with ``okg_expand_decay``
     score attenuation.
  4. **Multi-view rerank → RRM fusion** — Qwen3-Reranker scores the
     same candidate pool under three queries (configurable via
     ``rerank_views``):
       - ``narrow`` = ``ori_question`` (raw lexical) → L1 specialist
       - ``mid``    = anchored seed_query (full)     → L2 specialist
       - ``wide``   = anchored topic+conditions only → L3 specialist
     Rankings fuse via Reciprocal Rank MAX (``rerank_fusion="rrm"``):
     ``score(d) = max_v 1/(k + rank_v(d))``. Each doc keeps the rank
     of the view that loves it most → no averaging dilution. Validated
     +3.1pp OV R@10 / +2.0pp OV FC@10 / +13.9pp L3 R@10 vs single-view
     mid on RegOps (``experiments/multi_view_rerank_3view_rrm_pool50_cached/``).
     ``rerank_fusion="rrf"`` recovers RAG-Fusion-style sum aggregation.
  5. **Visited filter** — drop hits already seen by the caller, fill
     up to ``top_k`` from the fused tail.

Module surface:

  - :class:`WalkerRetrievalPipeline` — the full pipeline.
  - :class:`TopicExtractor` — topic + conditions anchorer for raw
    strings (uses ``TOPIC_ANCHORING`` from ``src.utils.prompts.refwalker``
    and ``QueryAnalysis`` from ``src.ground.schemas``). On by default,
    lazy-initialised — no LLM client until first use.
  - :func:`build_tagged_query` — render an anchored dict to the tagged
    seed query string. Useful for callers (e.g. eval) that pre-anchor
    in batch and pass strings directly.
  - :func:`format_hits_for_walker` — render hits for the Walker prompt.

Configuration knobs:

  - ``rerank_pool`` — how many seeds the first stage fetches.
  - ``okg_expand_seed`` — of those seeds, how many drive the 1-hop walk.
  - ``okg_expand_decay`` — neighbour score = seed × decay.
  - ``expand_edge_types`` — which OKG edge types qualify (PART_OF
    intentionally excluded — it's hierarchical containment).
  - ``topic_extractor`` — :class:`TopicExtractor` instance. **On by
    default** (lazy). Pass ``topic_extractor=None`` to disable when
    callers always supply pre-anchored data.
"""
